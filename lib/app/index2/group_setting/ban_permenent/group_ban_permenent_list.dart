...
class GroupBanPermenentList extends StatefulWidget {
  String _title;
  var _pageparam;

  GroupBanPermenentList(this._title, this._pageparam);

  _GroupBanPermenentList createState() => _GroupBanPermenentList(this._title, this._pageparam);
}

class _GroupBanPermenentList extends State<GroupBanPermenentList> {
  String _title;
  var _pageparam;

  _GroupBanPermenentList(this._title, this._pageparam);
  ...
  Future<void> get_data(BuildContext context, String gid) async {
    ...
    String ret = await Net.Post(Config.Url, Url_group_setting.Group_ban_list, null, post, null);
        ...
        child: ListView.builder(
          itemBuilder: (context, index) {
            ...
            return new Slidable(
              ...
              child: ListTile(
                leading: null,
                title: Text(_data["user_info"]["nickname"].toString()),
                subtitle: Text("操作人：" + _data["user_id"].toString()),
                trailing: Column(
                  children: [
                    Text("加入时间"),
...
Future<bool> delete_data(BuildContext context, String uid, gid) async {
  ...
  String ret = await Net.Post(Config.Url, Url_group_setting.Group_ban_delete, null, post, null);
  Map json = jsonDecode(ret);
  if (Auth.Return_login_check(context, json)) {
    if (Ret.Check_isok(context, json)) {
